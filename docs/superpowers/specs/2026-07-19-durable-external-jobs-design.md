...
An existing `(tenant, operation_scope, key_digest)` row with the same request hash returns the original job; a different hash returns an idempotency conflict. For a new job, the service generates source/job IDs, encrypts source with tenant and source ID as AES-GCM associated data, writes ciphertext to an object lifecycle abstraction, and persists only object key, digest, length, key version, and nonce. Database commit publishes the job and idempotency response. Commit failure deletes the staged source. Object cleanup failure is reported for retry without exposing object keys.
...
Claim uses one transaction and `SELECT ... FOR UPDATE SKIP LOCKED`, ordered by `next_attempt_at, created_at, id`. It selects due `QUEUED` jobs or expired `RUNNING` jobs, expires the old attempt, increments `attempt_no`, generates an unguessable lease token, and inserts an attempt row. The lease token, attempt number, worker ID, and active lease form the fencing predicate for heartbeat, completion, and retry.
