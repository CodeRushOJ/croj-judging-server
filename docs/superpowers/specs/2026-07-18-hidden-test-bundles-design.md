...
The runner executes cases in manifest order through one compile-once batch. Each request contains immutable source, case stdin, expected output for exact checking, version limits, and one selected Ready endpoint. Token expected output is canonicalized as length-prefixed Unicode-whitespace tokens and immediately reduced to SHA-256; only that digest is retained and sent, allowing sandbox-side first-error stop without exposing or retaining the raw hidden answer. Judging canonicalizes and hashes actual output with the same rule for its independent check. `Accepted` continues; CE/WA/TLE/MLE/RE/OLE stop immediately. Exact repeats the sandbox comparison against retained raw expected output so an empty expected output cannot bypass comparison. OLE temporarily maps to callback v1 `RUNTIME_ERROR` and retains only the OLE label in its summary. Time and memory aggregate by maximum, and callback text contains only bounded case IDs/verdict summaries.
