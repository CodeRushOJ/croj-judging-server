...
func TestOpenAPIContractPinsSecurityHeadersAndAsynchronousSemantics(t *testing.T) {
	...
	capabilityLimits := document.Components.Schemas["CapabilityLimits"].Value
	for _, field := range []string{"maxTimeLimitMillis", "maxMemoryLimitMiB"} {
		required := false
		for _, candidate := range capabilityLimits.Required {
			if candidate == field {
				required = true
				break
			}
		}
		property := capabilityLimits.Properties[field]
		if !required || property == nil || property.Value == nil || property.Value.Min == nil || *property.Value.Min != 1 {
			t.Errorf("CapabilityLimits.%s must be required with minimum 1", field)
		}
	}
...
func TestOpenAPIDocumentationAdvertisesDraftContract(t *testing.T) {
	...
	for _, required := range []string{"api/openapi.yaml", "Draft", "外部 listener 默认关闭", "EXTERNAL_API_ENABLED=true"} {
